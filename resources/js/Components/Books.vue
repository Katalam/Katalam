<script setup lang="ts">
import { Link } from "@inertiajs/inertia-vue3";
</script>

<template>
    <h2>Books I've read that I can recommend</h2>
    <ul>
        <li>
            Communications Networks by A. Leon-Garcia & I. Widjaja ed. 2nd,
            McGraw-Hill, 2004
        </li>
        <li>
            The web application hacker’s handbook: finding and exploiting
            security by Stuttard D. ed. 2nd, Wiley, 2011
        </li>
        <li>
            Database Systems - The Complete Book by Hector Garcia-Molina,
            Jeffrey D. Ullman, Jennifer Widom, ed. 2nd, Pearson Education
            International, 2014
        </li>
        <li>
            The Foundations of Cryptography by Oded Goldreich, ed. 2nd,
            Cambridge University Press, 2003
        </li>
        <li>
            Certified Tester - Foundation Level by Andreas Spillner & Tilo Linz,
            ed. 6th, dpunkt, 2018
        </li>
        <li>
            Operating Systems: Internals and Design Principles by William
            Stallings, ed. 9th, Pearson, 2017
        </li>
        <li>
            Computer Networking: A Top-down Approach by J. F. Kurose & K. W.
            Ross, ed. 6th, Pearson, 2012
        </li>
        <li>
            Numerical Methods in Engineering with Python by Kiusalaas & Jaan,
            ed. 2nd, Cambridge University Press, 2010
        </li>
    </ul>

    <h3>Laravel related</h3>
    <ul>
        <li>
            Domain-Driven Design with Laravel by Martin Joo (
            <Link
                target="_blank"
                href="https://domain-driven-design-laravel.com"
                >https://domain-driven-design-laravel.com
            </Link>
            )
        </li>
        <li>
            Design for Developers by Adrian Twarog (
            <Link target="_blank" href="https://enhanceui.com"
                >https://enhanceui.com
            </Link>
            )
        </li>
        <li>
            Advanced Inertia by Boris Lepikhin (
            <Link target="_blank" href="https://advanced-inertia.com"
                >https://advanced-inertia.com
            </Link>
            )
        </li>
        <li>
            Battle Ready Laravel by Ash Allen (
            <Link target="_blank" href="https://battle-ready-laravel.com/"
                >https://battle-ready-laravel.com/
            </Link>
            )
        </li>
        <li>
            Design for Developer by Adrian Twarog & George Moller (
            <Link target="_blank" href="https://www.enhanceui.com/"
                >https://www.enhanceui.com/
            </Link>
            )
        </li>
        <li>
            Performance with Laravel by Martin Joo (
            <Link target="_blank" href="https://laravel-performance.com/"
                >https://laravel-performance.com/
            </Link>
            )
        </li>
    </ul>
</template>
